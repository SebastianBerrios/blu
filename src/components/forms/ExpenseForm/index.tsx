...
export default function ExpenseForm({
  ...
  const { egresoCategories } = useTransactionCategories();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const category = egresoCategories.find((c) => c.id === Number(categoryId));
      ...
      logAudit({
        ...
        targetDescription: `Gasto: ${category?.name ?? ""} - ${description.trim()} - S/ ${numAmount.toFixed(2)}`,
        details: {
          tipo: "gasto",
          monto: numAmount,
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            {egresoCategories.length === 0 ? (
              <p className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                No hay categorías de egreso. Pídele a un administrador que cree una.
              </p>
            ) : (
              <select
                value={categoryId}
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              ...
              disabled={isSubmitting || egresoCategories.length === 0}
