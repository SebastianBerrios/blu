...
describe("buildCategoryPayload", () => {
  it("normaliza name a lowercase", () => {
    expect(
      buildCategoryPayload({ name: "POSTRES Especiales", tipo: null, target_margin: 30 }),
    ).toEqual({ name: "postres especiales", tipo: null, target_margin: 30 });
  });

  it("conserva target_margin válido y lo anula si no es positivo", () => {
    expect(
      buildCategoryPayload({ name: "x", tipo: null, target_margin: 25 }).target_margin,
    ).toBe(25);
    expect(
      buildCategoryPayload({ name: "x", tipo: null, target_margin: 0 }).target_margin,
    ).toBeNull();
    expect(
      buildCategoryPayload({ name: "x", tipo: null, target_margin: null }).target_margin,
    ).toBeNull();
  });

  it("conserva tipo cuando es 'postre' o 'bebida'", () => {
    expect(buildCategoryPayload({ name: "x", tipo: "postre", target_margin: 30 }).tipo).toBe("postre");
    expect(buildCategoryPayload({ name: "x", tipo: "bebida", target_margin: 30 }).tipo).toBe("bebida");
  });
  ...
  it("setea tipo=null para valores no válidos", () => {
    ...
    expect(buildCategoryPayload({ name: "x", tipo: null, target_margin: 30 }).tipo).toBeNull();
  });
});
...
describe("createCategory / updateCategory", () => {
  ...
  it("createCategory inserta en categories", async () => {
    ...
    await createCategory({ name: "bebidas", tipo: "bebida", target_margin: 25 });
    expect(sb.insertCalls).toEqual([
      { table: "categories", payload: { name: "bebidas", tipo: "bebida", target_margin: 25 } },
    ]);
  });
  ...
  it("updateCategory hace update con .eq('id', n)", async () => {
    const sb = makeMockSupabase();
    vi.mocked(createClient).mockReturnValue(sb.client as never);
    await updateCategory(7, { name: "postres", tipo: "postre", target_margin: 30 });
    expect(sb.updateCalls).toEqual([
      {
        table: "categories",
        payload: { name: "postres", tipo: "postre", target_margin: 30 },
        filters: [["id", 7]],
      },
    ]);
  });
  ...
  it("createCategory propaga error", async () => {
    ...
    vi.mocked(createClient).mockReturnValue(sb.client as never);
    await expect(createCategory({ name: "x", tipo: null, target_margin: 30 })).rejects.toBeTruthy();
  });
});
