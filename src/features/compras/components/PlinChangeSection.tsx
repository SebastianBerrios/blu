...
interface PlinChangeSectionProps {
...
export default function PlinChangeSection({
  hasPlinChange,
  plinChange,
  onToggle,
  onChange,
  isSubmitting,
}: PlinChangeSectionProps) {
  return (
    <div className="border border-slate-200 rounded-lg p-4 bg-slate-50/50">
      <label className="flex items-center gap-3 cursor-pointer">
        <input
          type="checkbox"
          checked={hasPlinChange}
          onChange={(e) => {
      ...
      {hasPlinChange && (
        <div className="mt-3">
          ...
          <p className="text-xs text-slate-500 mt-1.5">
            El vuelto que el vendedor te devolvió por Plin (se registra en la Cuenta Bancaria)
          </p>
        </div>
      )}
    </div>
